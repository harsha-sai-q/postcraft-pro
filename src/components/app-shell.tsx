...
export function AppShell({ children, email }: { children: React.ReactNode; email: string }) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-100 via-slate-50 to-white md:flex">
      ...
      <main className="flex-1">
        <header className="sticky top-0 z-20 border-b border-slate-200 bg-white/95 p-4 backdrop-blur md:hidden">
          <div className="flex items-center justify-between">
            <p className="font-semibold tracking-tight text-slate-900">PostCraft Pro</p>
              ...
              {nav.map(({ label, href }) => (
                <Link
                  ...
                  className={`rounded-lg border p-2 text-sm ${
                    pathname === href ? "border-slate-900 bg-slate-900 text-white" : "border-slate-200 bg-white text-slate-700"
                  }`}
